  ...
  if ((V.Pv.N >0) || (IA.VIA.PV > 0))     //si un des vaisseaux n'est pas encore détruit
  {
    ...
    for (int i=0; i<=1; i++) {                //pour les différents missiles appartenant à l'arme de l'IA //////////////////////////////////////////////////////
      if ((IA.MEquiper == Miss.M[i][0]) && (IA.MRTer== true) && (IA.Tir == true)) {          
        ...
        noFill();
        imageMode(CORNER);
        //println(Missile[1].Pos.dist(V.AvPos(Viser)));
...
{
  for (int i=0; i<=1; i++) {
    copy(Tex.Ico[15+i], 300*int((frameCount - F[11]) / (frameRate *100)), i*300, 300, 300, int(Missile[0].Pos.x), int(Missile[0].Pos.y), 30, 30); 
    stroke(255);
    noFill();
